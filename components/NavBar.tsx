...
export function NavBar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 w-full bg-white dark:bg-gray-900 transition-all duration-700 ease-in-out
    ${scrolled ? "opacity-0 -translate-y-full pointer-events-none" : "opacity-100 translate-y-0"}
  `}
      >
        {/* Glass morphism container */}
        <div
          className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-500 ${scrolled ? 'bg-white/90 backdrop-blur-xl shadow-2xl' : 'bg-white/70 backdrop-blur-md shadow-lg'
        ...
        <div
          className={`md:hidden absolute top-full left-0 right-0 mt-0 sm:mt-3 mx-2 sm:mx-4 transition-all duration-500 ${menuOpen
